...
const SYNC_PACKET = util_1.toByteArray("\x07\x07\x12 UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU");
...
class EspLoader {
    constructor(serialPort, options) {
        ...
        this.inputBuffer = new util_1.Uint8Buffer(64);
    ...
    async checkCommand(opcode, buffer, checksum = 0, timeout = DEFAULT_TIMEOUT) {
        ...
        if (status[0] == 1) {
            if (status[1] == ROM_INVALID_RECV_MSG) {
                throw "Invalid (unsupported) command " + util_1.toHex(opcode);
            }
            else {
                throw "Command failure error code " + util_1.toHex(status[1]);
            }
        }
        return { value, data };
    }
    ...
    async flashData(binaryData, offset = 0, progressCallback = undefined, encrypted = false) {
        ...
        while (filesize - position > 0) {
            if (this.options.debug) {
                this.logger.debug("Writing at " + util_1.toHex(address + seq * flashWriteSize, 8) + "... (", (seq + 1) / blocks, "%)");
            }
            if (progressCallback) {
    ...
    async flashBegin(size = 0, offset = 0, encrypted = false) {
        ...
        this.logger.log("Erase size", eraseSize, " blocks ", numBlocks, " block size ", flashWriteSize, " offset " + util_1.toHex(offset, 4) + ", encrypted " + (encrypted ? "yes" : "no"));
            ...
            for (const [start, end] of [
                ...
                if (load_start < end && load_end > start) {
                    throw ("Software loader is resident at " +
                        util_1.toHex(start, 8) +
                        "-" +
                        util_1.toHex(end, 8) +
                        ". " +
                        "Can't load binary at overlapping address range " +
                        util_1.toHex(load_start, 8) +
                        "-" +
                        util_1.toHex(load_end, 8) +
                        ". " +
